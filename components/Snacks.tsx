...
const images = [
  { src: "/images/photos/popcorn-3dglasses.jpg", label: "Popcorn & 3D Glasses" },
  { src: "/images/photos/fanta-popcorn-slushie.webp", label: "Fresh Slushies & Popcorn" },
...
export default function Snacks() {
  ...
  return (
    <>
      ...
      <section id="snacks" className="relative overflow-hidden bg-ink py-20 sm:py-28 md:py-36 text-white border-t border-white/10">
        {/* Ambient Glows */}
        <div className="pointer-events-none absolute top-10 right-10 w-[500px] h-[500px] bg-[radial-gradient(circle,_var(--tw-gradient-stops))] from-orange/15 via-transparent to-transparent blur-3xl" />
        <div className="pointer-events-none absolute bottom-10 left-10 w-[500px] h-[500px] bg-[radial-gradient(circle,_var(--tw-gradient-stops))] from-gold/15 via-transparent to-transparent blur-3xl" />
            ...
            {images.map((img, i) => (
              <Reveal key={img.src} delay={(i % 4) as 0 | 1 | 2 | 3 | 4}>
                <div className="group relative aspect-[4/3] overflow-hidden rounded-3xl border border-white/15 bg-ink-soft shadow-xl transition-all duration-500 hover:-translate-y-1.5 hover:border-gold/40">
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-ink/85 via-ink/20 to-transparent flex items-end p-5 opacity-90 transition-opacity duration-300 group-hover:opacity-100">
            ...
            {t.snacks.items.map((item, i) => (
              <Reveal key={item.title} delay={(i % 4) as 0 | 1 | 2 | 3 | 4}>
                <div className="group flex items-start gap-4 rounded-2xl border border-white/10 bg-ink-soft/75 p-5 backdrop-blur-md transition-all duration-300 hover:-translate-y-1 hover:border-orange/40 hover:shadow-lg">
                  <span className="mt-1.5 flex h-3 w-3 shrink-0 rounded-full bg-gradient-to-r from-orange to-gold shadow-sm" />
                  <div>
                    <h3 className={`font-display text-lg uppercase text-white group-hover:text-gold-soft transition-colors !leading-[1.25] ${ethiopic}`}>{item.title}</h3>
                    <p className={`mt-1 text-sm leading-relaxed text-white/75 font-light ${ethiopic}`}>{item.description}</p>
                  </div>
                </div>
              </Reveal>
            ))}
